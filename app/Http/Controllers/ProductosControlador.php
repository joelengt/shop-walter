...
class ProductosControlador extends Controller
{
    ...
    public function BorrarProducto(Request $request,$id_producto)
    {
        if(!Auth::check()) {
            return Redirect::to('/admin/login');
        }else {
        ...
        if(!Auth::check()) {
            ...
            if ($request->method() == 'GET') {
                ...
                $validator = Validator::make($request->all(), [
                    'nombre_producto' => 'required|string|min:10|max:255',
                ...
                if ($validator->fails()) {
                    ...
                    return redirect(route('editar-producto-admin',['id_producto'=>$id_producto]))
        ...
        if(!Auth::check()) {
            ...
            if ($request->method() == 'GET') {
                ...
                if ($validator->fails()) {
                    ...
                    return redirect(route('agregar-producto-admin'))
                        ->withErrors($validator)
                        ->withInput();
                }
