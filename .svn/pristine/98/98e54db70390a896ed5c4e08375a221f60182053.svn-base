        ...
        if(!Auth::check()) {
            ...
            if ($request->method() == 'GET') {
                ...
                $validator = Validator::make($request->all(), [
                    'imagen_producto' => 'required|file|image',
                    'habilitado' => 'required|in:0,1',
                ]);

                if ($validator->fails()) {

                    return redirect(route('editar-producto-imagen-admin',['id_producto' => $id_producto,'id_imagen' => $id_imagen]))
        ...
        if(!Auth::check()) {
            ...
            if ($request->method() == 'GET') {
                ...
                if ($validator->fails()) {
                    ...
                    return redirect(route('agregar-producto-imagen-admin',['id_producto'=>$id_producto]))
                        ->withErrors($validator)
                        ->withInput();
                }
